...
namespace RPGGame
{
    ...
    {
        Random random;
        public ForwardEnemy(Texture2D image, Texture2D arrow, Vector2 position, Color tint, Vector2 origin, Vector2 scale, Vector2 speed, SpriteEffects effect,
        ...
        public override void Update(GameTime gameTime, Rectangle boundary)
        {
            ...
            if (Math.Sqrt(Math.Pow((knight.Position.X - Position.X), 2) + Math.Pow((knight.Position.Y - Position.Y), 2)) <= 200)
            {
                ...
                //if (Position.Y < knight.Position.Y && xDistance < yDistance && state != EnemyStates.fightingForward)
                //{
                //    currentFrameIndex = 0;
                //    state = EnemyStates.fightingForward;
                //}
            }
            else
            {
                currentFrameIndex = 0;
                state = EnemyStates.idleDown;
            }
        ...
        public override void Draw(SpriteBatch spriteBatch)
        {
            ...
            spriteBatch.DrawString(Font, $"{enemyHealthPoints}", new Vector2(Position.X + 17, Position.Y - 15), Color.Black);
        }
    }
}
