...
namespace RPGGame
{
    class BackwardEnemy : Enemy
    {
        Random random;
        public BackwardEnemy(Texture2D image, Texture2D arrow, Vector2 position, Color tint, Vector2 origin, Vector2 scale, Vector2 speed, SpriteEffects effect,
        ...
        public override void Update(GameTime gameTime, Rectangle boundary)
        {
            ...
            if (Math.Sqrt(Math.Pow((knight.Position.X - Position.X), 2) + Math.Pow((knight.Position.Y - Position.Y), 2)) <= 250)
            {
                arrowDirection = new Vector2(random.Next(-1,3), random.Next(-3,-1));
                ...
                //if (Position.Y > knight.Position.Y && xDistance < yDistance && state != EnemyStates.fightingBackward)
                //{
                //    currentFrameIndex = 0;
                //    state = EnemyStates.fightingBackward;
                //}
            }
            else
            {
                currentFrameIndex = 0;
                state = EnemyStates.idleUp;
            }
        ...
        {
            spriteBatch.DrawString(Font, $"{enemyHealthPoints}", new Vector2(Position.X - 20, Position.Y - 55), Color.Black);
